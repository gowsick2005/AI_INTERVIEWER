...
old_ring_svg = '<svg className="score-ring-svg" viewBox="0 0 120 120">'
new_ring_svg = '<svg className="score-ring-svg" viewBox="0 0 200 200">'
tsx_updated = tsx_updated.replace(old_ring_svg, new_ring_svg)

old_circles = '<circle cx="60" cy="60" r="50" className="score-ring-bg" />\\n                        <circle cx="60" cy="60" r="50" className="score-ring-fill" style={{ strokeDasharray: \'314\', strokeDashoffset: `${314 * (1 - score / 100)}` }} />'
new_circles = '<circle cx="100" cy="100" r="88" className="score-ring-bg" />\\n                        <circle cx="100" cy="100" r="88" className="score-ring-fill" style={{ strokeDasharray: \'553\', strokeDashoffset: `${553 * (1 - score / 100)}` }} />'
# Try simple replace for safety
tsx_updated = tsx_updated.replace('<circle cx="60" cy="60" r="50" className="score-ring-bg" />', '<circle cx="100" cy="100" r="88" className="score-ring-bg" />')
tsx_updated = tsx_updated.replace("style={{ strokeDasharray: '314', strokeDashoffset: `${314 * (1 - score / 100)}` }}", "style={{ strokeDasharray: '553', strokeDashoffset: `${553 * (1 - score / 100)}` }}")
tsx_updated = tsx_updated.replace('r="50" className="score-ring-fill"', 'r="88" className="score-ring-fill"')
...
if start_idx_html != -1 and end_idx_html != -1:
    ...
    html_updated = html_updated.replace('<svg class="score-ring-svg" viewBox="0 0 120 120">', '<svg class="score-ring-svg" viewBox="0 0 200 200">')
    html_updated = html_updated.replace('<circle cx="60" cy="60" r="50" class="score-ring-bg" />', '<circle cx="100" cy="100" r="88" class="score-ring-bg" />')
    html_updated = html_updated.replace('<circle cx="60" cy="60" r="50" class="score-ring-fill" id="results-ring-element" />', '<circle cx="100" cy="100" r="88" class="score-ring-fill" id="results-ring-element" />')
    
    # Update JS strokeDashoffset multipliers in HTML
    html_updated = html_updated.replace("const offset = 314 * (1 - current.score / 100);", "const offset = 553 * (1 - current.score / 100);")
    
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(html_updated)

print("Styles scaled and score rings updated dynamically!")
